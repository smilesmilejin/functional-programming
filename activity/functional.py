...
def shortest_word(words):
    ...
    return reduce(lambda shortest, word: word if len(word) < len(shortest) else shortest, words)   
...
def sorted_valid_passwords(passwords):
    # pass
    passwords_list = list(filter(lambda password: not password.isalpha(), passwords))
    return sorted(passwords_list, key=len)
